...
# add this after the params$pdf_mode block in the setup chunk
# small inline favicon to prevent a /favicon.ico 404 during chrome_print
cat('<link rel="icon" href="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVQYV2NgYAAAAAMAAWgmWQ0AAAAASUVORK5CYII=">\n')

# Read in all data and initialize a CV printer object
# params <- list(lang = 'en', sheet = 'cv')
cv <- create_cv_object(
...
cv %>% print_experience("professional_experience", n_top = 4)
```
...
cv %>% print_skill_bars(section_ids = "ai")
```
...
```{r professional_experience_2}
cv %>% print_experience("professional_experience", n_down = 2)
```
...
```{r awards}
if (params$lang == "en") {
  cat("## Awards {data-icon=award}\n")
} else {
  cat("## Premi {data-icon=award}\n")
}
cv %>% print_experience("award")
```
