...
create_cv_object <- function(data_location, sheet = "cv") {
  ...
  if (grepl("^cv", sheet)) {
    cv$experiences <- googlesheets4::read_sheet(
      data_location,
      sheet = sheet,
      col_types = "c"
    ) %>%
  ...
  cv$text_blocks <- googlesheets4::read_sheet(
    data_location,
    sheet = "text_blocks",
    col_types = "c"
  )
  cv$skills <- googlesheets4::read_sheet(
    data_location,
    sheet = "skills",
    col_types = "c"
  )
  cv$contacts <- googlesheets4::read_sheet(
    data_location,
    sheet = "contacts",
    col_types = "c"
  )
...
>{skill}</div>"
  ...
  if (resume) {
    cv$skills %>%
      ...
      dplyr::distinct() %>%
      dplyr::mutate(
        level = as.numeric(stringr::str_replace_all(level, "\\,", "\\."))
      ) %>%
